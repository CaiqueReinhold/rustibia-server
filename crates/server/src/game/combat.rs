...
pub fn plan_spell_attack(
    ...
    if spell.weapon_required && matches!(player.weapon_type(), WeaponType::None) {
        return Err(SpellCastingDenyReason::NoWeapon);
    }
    ...
    let value = roll_power(player, &spell.power, roll, spell.weapon_required);
...
pub fn get_damage_visuals(
    ...
    let effect = match damage.element {
        ...
        CombatElement::Mana => GAME_CONFIG.effect_ids.mana_hit,
    };
    let color = match damage.element {
        ...
        CombatElement::Mana => GAME_CONFIG.text_colors.blue,
    };
    (effect, color)
}
